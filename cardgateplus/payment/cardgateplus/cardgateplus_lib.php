...
class cgp_generic {
	...
	private $prefix;
	private $language;
        //private $_url = 'https://gateway.cardgateplus.com/';
        private $_url = 'https://ralph.api.curopayments.dev/gateway/cardgate/';
	...
	{
		if (!$this->logToFile) return false;
	...
	function generateBankHtml(){
		...
		foreach( $aIssuers as $id => $name ) {
			...
			if( isset($_COOKIE['cgp_bank']) && $id == $_COOKIE['cgp_bank'] ){ $html .= ' selected="selected" ';}
			$html .='">'.$name.'</option>';
		}
		$html .= '</select>';
		return $html;
